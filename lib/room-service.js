...
async function createRoom(params) {
  ...
  const { phone, channel, source, name, userAgent = "Unknown", ipAddress = "Unknown", createdFrom = "webhook", userId, tags } = params
...
// Get room connections with socket validation and roles
async function getRoomConnectionsWithRoles(roomId) {
  try {
    const db = await getDatabase()
    const room = await db.collection("rooms").findOne(
      { _id: new ObjectId(roomId) },
    ...
    const connectedSockets = room?.connectedSockets || []

    // Get socket roles from users collection
    const socketRoles = await Promise.all(
      connectedSockets.map(async (socketId) => {
    ...
    return {
      connectedSockets: socketRoles,
      status: room?.status || "closed",
      connectedCount: connectedSockets.length
    }
  } catch (error) {
    console.error("Error getting room connections with roles:", error)
    throw new Error(`Failed to get room connections with roles: ${error.message}`)
  }
}
